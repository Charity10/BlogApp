...
const BlogCard = ({ blog: { title, desc, imageUrl, likes, authorId, _id } }) => {
  ...
  const handleLike = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_LIVE_APP_URL}/api/blog/${_id}/like`, {
        headers: {
    ...
    <div className={classes.container}>
      <div className={classes.wrapper}>
        <Link className={classes.imgContainer} href={`/blog/${_id}`}>
          <Image src={imageUrl}  alt="blog-image" width="350" height="350" />
        </Link>
        <div className={classes.blogData}>
          <div className={classes.left}>
            <h3>{title}</h3>
            <p>{desc}</p>
            <span>Created By: <span>1th of J
              anuary</span></span>
          </div>
